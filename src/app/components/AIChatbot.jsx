...
import React, { useState, useRef, useEffect, useCallback } from "react";
import { useVoiceAssistant } from "@/ai/voice/useVoiceAssistant";
...
export default function AIChatbot() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      text: "Hey! I’m your **AlumniConnect AI Assistant** 🤖🎓\n\nYou can **type or speak** to ask about careers, mentors, scholarships, events, or startups."
    }
  ]);
  const [input, setInput] = useState("");
  ...
  const { startListening, speak } = useVoiceAssistant((voiceText) => {
  ...
  const generateResponse = async (userText) => {
    ...
    return `You asked: **${userText}**\n\nThis feature is powered by our **AI-driven alumni intelligence system**.\n\nAsk more to explore 🚀`;
  };
  ...
  const handleSend = useCallback(
    async (text) => {
      ...
      // 🔊 SPEAK RESPONSE (clean text)
      speak(aiReply.replace(/[*#\n]/g, " "));
    },
    [speak]
  );
  ...
  return (
    <>
      ...
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="fixed bottom-6 right-6 z-50 bg-indigo-600 text-white p-4 rounded-full shadow-xl hover:bg-indigo-700"
      >
        <MessageIcon />
      </button>
      ...
      {/* CHAT WINDOW */}
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-[360px] h-[520px] bg-white rounded-xl shadow-2xl flex flex-col z-40 border">

          {/* HEADER */}
          <div className="bg-indigo-600 text-white p-4 rounded-t-xl font-semibold">
            AlumniConnect AI (Voice Enabled)
          </div>
